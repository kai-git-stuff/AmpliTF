from amplitf.fitting.TreeAmplitudes import TreeAmplitude, TreeResonance
...
class dalitz_decay:
    ...
    def chain3(self,smp:PhaseSpaceSample,ld,la,lb,lc,bls_in:TreeAmplitude,bls_out:TreeAmplitude,ResonanceSpin,name):
        """
            ...
            for lA in ResonanceHelicities:           
                ...
                for la_,lb_,lc_ in helicities_abc:
                    ...
                    H_A_c = ( atfi.sqrt(phasespace_factor(self.md,sgma3,self.mc)* phasespace_factor(sgma3,self.ma,self.mb)) * 
                        helicity_coupling_times_d(theta_hat,self.sd,sA,self.sc,lA,lc_,ld,bls_in(kwargs)) 
                        )
                    w_d_1 = atfi.cast_complex(wigner_small_d(theta,self.sd,ld,lA-lc_))  * (-1)**((self.sc-lc_)/2)
                    H_A_c = atfi.cast_complex(helicity_couplings_from_ls(self.sd,sA,self.sc,lA,lc_,bls_in(kwargs))) * w_d_1

                    # Rotation in the isobar system
                    # angle between A momentum (isobar) and lmbda_c in rest frame of Isobar 
                    H_a_b =  helicity_coupling_times_d(theta,sA,self.sa,self.sb,la_,lb_,lA,bls_out(kwargs))
