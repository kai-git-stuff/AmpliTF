...
class Baryonic3BodyPhaseSpace(DalitzPhaseSpace):
    ...
    def inside(self, x) -> bool:
        ...
        if self.macrange:
            ...
            inside = tf.logical_and(
                inside,
                tf.logical_and(
                    tf.greater(m2ac, self.macrange[0] ** 2),
        ...
        inside_phsp = tf.logical_and(
            inside, tf.logical_and(tf.greater(m2bc, m2bc_min), tf.less(m2bc, m2bc_max))
        )

        inside_theta = tf.logical_and(
            tf.greater(costhetaa, -1.0), tf.less(costhetaa, 1.0)
        )
        inside_phi = tf.logical_and(
            tf.logical_and(tf.greater(phia, -1.0 * math.pi), tf.less(phia, math.pi)),
            tf.logical_and(tf.greater(phibc, -1.0 * math.pi), tf.less(phibc, math.pi)),
        )
    ...
    def final_state_momenta(self, m2ab, m2bc, costhetaa, phia, phibc):
        ...
        p3b = atfk.vector(
            p_b * Sqrt(1.0 - cos_theta_b ** 2), atfi.zeros(p_b), -p_b * cos_theta_b
        )
        p3c = atfk.vector(
            -p_c * Sqrt(1.0 - cos_theta_c ** 2), atfi.zeros(p_c), -p_c * cos_theta_c
        )
