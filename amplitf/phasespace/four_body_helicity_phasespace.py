...
class FourBodyHelicityPhaseSpace(BasePhaseSpace):
    ...
    def simple_inside(self, x):
        ...
        inside = tf.logical_and(
            tf.logical_and(
                tf.greater(ctha, self.costhamin), tf.less(ctha, self.costhamax)
            ),
            tf.logical_and(
                tf.greater(cthb, self.costhbmin), tf.less(cthb, self.costhbmax)
            ),
        )
        inside = tf.logical_and(
            inside, tf.logical_and(tf.greater(phi, -math.pi), tf.less(phi, math.pi))
        )

        mb1b2max = atfi.min(
            atfi.cast_real(self.mb1b2max), atfi.cast_real(self.md) - ma1a2
        )

        inside = tf.logical_and(
            inside,
            tf.logical_and(
                tf.greater(ma1a2, self.ma1a2min), tf.less(ma1a2, self.ma1a2max)
            ),
        )
        inside = tf.logical_and(
            inside,
            tf.logical_and(tf.greater(mb1b2, self.mb1b2min), tf.less(mb1b2, mb1b2max)),
        )
    ...
    def unfiltered_sample(self, size, maximum=None):
        ...
        v = [
            tf.random.uniform(
                [size], self.ma1a2min, self.ma1a2max, dtype=atfi.fptype()
            ),
            tf.random.uniform(
                [size], self.mb1b2min, self.mb1b2max, dtype=atfi.fptype()
            ),
            tf.random.uniform(
                [size], self.costhamin, self.costhamax, dtype=atfi.fptype()
            ),
            tf.random.uniform(
                [size], self.costhbmin, self.costhbmax, dtype=atfi.fptype()
            ),
    ...
    def final_state_momenta(self, x):
        ...
        p3A = atfk.rotate_euler(
            atfk.vector(zeros, zeros, pA), zeros, atfi.acos(ctha), zeros
        )
        p3B = atfk.rotate_euler(
            atfk.vector(zeros, zeros, pB), zeros, atfi.acos(cthb), phi
        )
