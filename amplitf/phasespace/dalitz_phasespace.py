...
class DalitzPhaseSpace(BasePhaseSpace):
    ...
    def cos_helicity_ab(self, sample):
        ...
        """
        return atfk.cos_helicity_angle_dalitz(
    ...
    def from_square_dalitz_plot(self, mprimeac, thprimeac):
        ...
        m2AC = (
            0.25
            * (
                self.maxac ** 0.5 * atfi.cos(math.pi * mprimeac)
                + self.maxac ** 0.5
                - self.minac ** 0.5 * atfi.cos(math.pi * mprimeac)
                + self.minac ** 0.5
            )
            ** 2
        )
    ...
    def square_dalitz_plot_jacobian(self, sample):
        ...
        eAcmsAC = (
            ...
            * (
                mACSq
                - tf.cast(self.mc2, atfi.fptype())
                + tf.cast(self.ma2, atfi.fptype())
            )
            / mAC
        )
        eBcmsAC = (
            atfi.const(0.5)
            * (
                tf.cast(self.md, atfi.fptype()) ** 2.0
                - mACSq
                - tf.cast(self.mb2, atfi.fptype())
            )
            / mAC
        )
