...
class FourBodyAngularPhaseSpace(BasePhaseSpace):
    ...
    def inside(self, x):
        ...
        inside = tf.logical_and(
            tf.logical_and(tf.greater(cos1, -1.0), tf.less(cos1, 1.0)),
            tf.logical_and(tf.greater(cos2, -1.0), tf.less(cos2, 1.0)),
        )
        inside = tf.logical_and(
            inside, tf.logical_and(tf.greater(phi, 0.0), tf.less(phi, 2.0 * math.pi))
        )
        return inside
    ...
    def rectangular_grid_sample(self, size_cos_theta1, size_cos_theta2, size_phi):
        ...
        v1 = (
            mgrid[0:size_cos_theta1, 0:size_cos_theta2, 0:size_phi][0]
            * 2.0
            / float(size_cos_theta1)
            - 1.0
        )
        v2 = (
            mgrid[0:size_cos_theta1, 0:size_cos_theta2, 0:size_phi][1]
            * 2.0
            / float(size_cos_theta2)
            - 1.0
        )
        v3 = (
            mgrid[0:size_cos_theta1, 0:size_cos_theta2, 0:size_phi][2]
            * 2.0
            * math.pi
            / float(size_phi)
        )
        v = [
            v1.reshape(size).astype("d"),
            v2.reshape(size).astype("d"),
            v3.reshape(size).astype("d"),
        ]
        x = tf.stack(v, axis=1)
        return tf.boolean_mask(x, self.inside(x))
